...
def randCent(dataSet, k):
    # 随机初始化k个质心。每个质心的每一个维度都是在最大值和最小值之间随机。
    # 得到数据样本的维度
...
def biKmeans(dataSet, k, distMeas=distEclud):
    """二分k-means聚类算法。首先初始化所有数据为一整个簇。然后循环判断每一个簇。判断是否将簇进行二分。判断依据为二分后的SSE（误差平方和）最小。

    Args:
        dataSet (_type_): 用于聚类的数据集
        k (_type_): 选取k个质心
        distMeas (_type_, optional): 距离计算方法,默认欧氏距离distEclud()
...
if __name__ == '__main__':
    datMat = np.mat(loadDataSet('04_KMeans/data/testSet2.txt'))
